...
for run = 1 : Runs
    ...
    for i = 1 : npop
        FunctionValue(i,:)        = F_costfunction5obj(Population(i,:),P_customer,S_customer,Req_percus,Cost_req,N,M);     % calculate the objective function values
    end
    %-----------------------------------------------------------------------------------------
    % start iterations
    for Gene = 1 : Generations
        ...
        for i = 1 : npop
            NewFunctionValue(i,:)     = F_costfunction5obj(Offspring(i,:),P_customer,S_customer,Req_percus,Cost_req,N,M);    % calculate the objective function values
        end
    ...
    F_output(Population,toc,'IBEA-NRPmul_SPCBWM',P_customer,S_customer,Req_percus,Cost_req,N,M,run);
